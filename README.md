...
- Canvas server: web UI + REST API + WebSocket updates (default `http://localhost:4321`)
...
```bash
EXPRESS_SERVER_URL=http://localhost:4321 node dist/index.js
```
...
|----------|-------------|---------|
| `EXPRESS_SERVER_URL` | URL of the canvas server | `http://localhost:4321` |
...
```json
{
  "mcpServers": {
    "excalidraw": {
      ...
      "env": {
        "EXPRESS_SERVER_URL": "http://localhost:4321",
...
claude mcp add excalidraw --scope user \
  -e EXPRESS_SERVER_URL=http://localhost:4321 \
  -e ENABLE_CANVAS_SYNC=true \
...
codex mcp add excalidraw \
  --env EXPRESS_SERVER_URL=http://localhost:4321 \
...
All scripts respect `EXPRESS_SERVER_URL` (default `http://localhost:4321`) or accept `--url`.
